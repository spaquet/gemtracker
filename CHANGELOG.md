...
### Fixed
- **Release Workflow** - Sentry release commit linking no longer fails the job
  - `sentry-cli releases set-commits --auto` now passes `--ignore-missing`
  - Prevents CI failure when a previous release's commit SHA can't be resolved

## [v1.3.10] - 2026-06-22

### Added
- **Improved Ruby Version Detection** - Multi-source fallback strategy
  - Check `.ruby-version` or `.ruby` file in project root (highest priority)
  - Run `ruby --version` command in project directory
  - Parse Dockerfile for Rails projects (FROM ruby:X.Y.Z)
  - Extract from Gemfile.lock as fallback
